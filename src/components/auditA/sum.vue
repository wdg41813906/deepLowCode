  ...
  <div class="gatherlist">
    ...
    <el-form :inline="true">
      <el-form-item label="学院">
        <el-select v-model="departid" size="small">
          <el-option
...
export default {
  ...
  methods: {
    gourlClick (row) {
      this.$router.push({ name: 'filllistAudit', query: { num: beforeEncrypt('1'), userid: beforeEncrypt(row.userid), id: beforeEncrypt(this.task_code) } })
    },
    search (num) {
      let $this = this
      ...
      let posturl = $this.$apilist['departgather'] + '?signature=' + keyvalue;
      // return false;
      console.log(payload)
      this.loading = true
    ...
    level2departmap () {
      ...
      $this.$server.post(posturl, createvalue['fd']).then((data) => {
        ...
        this.departid = data.data[0].DepartmentId
        this.alltasklist()
    ...
    exportExcel () {
      ...
      let createvalue = $this.$createObj(payload);
      var keyvalue = createvalue['keyvalue'];
      let posturl = $this.$apilist['exportdepartgatherexcel'] + '?signature=' + keyvalue;
      // return false;
      console.log(payload)
