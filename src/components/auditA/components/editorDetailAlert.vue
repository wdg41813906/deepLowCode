      ...
      <div class="">
        ...
        >
          <el-form-item label="表单"
            >{{ form_data.title }} {{ form_data.labelPosition }}</el-form-item
          >
  ...
  methods: {
    ...
    calOther (val, indexs, assignval) {
      ...
      for (let i in assignval) {
        ...
        if ((assignval[i].ToValue).toString().split('')[assignval[i].ToValue.length - 1] == '.' || assignval[i].ToValue == '') break;
    ...
    subClick () {
      ...
      let payload = {
        ...
        form_code: this.other_data.FormCode,
    ...
    submitClick () {
      ...
      let payload = {
        corpid: this.$store.state.corpid,
        userid: this.$store.state.userid,
        form_code: this.form_data.FormCode,
