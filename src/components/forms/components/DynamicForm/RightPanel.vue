 ...
 <template>
  <div class="right-board">
      ...
      <el-scrollbar class="right-scrollbar">
        ...
        >
          ...
          <el-form-item
            v-if="
              ...
              activeData.tagIcon !== 'describe' &&
              activeData.tagIcon !== 'describeImg' &&
              activeData.tagIcon !== 'coefficient'
            "
            label="提示文字"
          >
            <el-input
          ...
          <template v-if="['select_number'].indexOf(activeData.tagIcon) > -1">
            ...
            >
              ...
              >
                ...
                />
                <span
                  class="el-input--small el-input cursor"
                  @click="select_n_click(item)"
                >
                  <span class="el-input__inner" v-if="isObj(item.value) == 1">{{
                    item.value
                  }}</span>
                  <span class="el-input__inner" v-else>{{
          ...
          <el-form-item
            v-if="activeData.tagIcon == 'coefficient'"
            label="系数值"
          >
            <el-input v-model="activeData.const_value"></el-input>
          </el-form-item>
          <el-form-item
            v-if="activeData.tagIcon == 'describeImg'"
            label="说明图片"
          >
            <div class="uploadimg_form" v-load="imgloading">
              <el-upload
                class="upload-demo"
                action=""
                :data="QiniuData"
                ...
                :http-request="
                  (file, fileList, item) => Upload(file, fileList, item)
                "
                multiple
                :file-list="imgList"
              >
        ...
        <el-form v-show="currentTab === 'form'" size="small" label-width="90px">
          ...
          <el-form-item label="汇总类型">
            <el-input v-model="formConf.summary_type" placeholder="汇总类型" />
          </el-form-item>
          <el-form-item label="顺序">
            <el-input v-model="formConf.sort" placeholder="顺序" />
          </el-form-item>
    ...
    <select-add-fun
      ...
      :select_num_flag="select_num_flag"
      :code="code"
      :funcnum="funcnum"
      :funclist="activeData"
      @back="backSelectAdd"
      @subBack="subBackSelect"
    ></select-add-fun>
  </div>
</template>
...
import SelectAddFun from '../select_add_fun'
import {
  inputComponents,
  selectComponents,
  layoutComponents
...
import Select_add_fun from '../select_add_fun.vue';
...
export default {
  components: {
    TreeNodeDialog,
    IconsDialog,
    draggable, FuncAdd, SelectAddFun
  },
  ...
  watch: {
    ...
    activeData (val) {
      ...
      for (let i in this.activeData.str_id_list) {

        if ((this.activeData.strlist[i].substr(0, 1) == '#' && this.activeData.strlist[i].substr(-1) == '#') || (this.activeData.strlist[i].substr(0, 2) == '"#' && this.activeData.strlist[i].substr(-1) == '"')) {
          if (strlist.code.indexOf(this.activeData.str_id_list[i]) > -1 && strlist.is_assign[strlist.code.indexOf(this.activeData.str_id_list[i])] == true) {
            this.activeData.strlist[i] = '#' + strlist.val[strlist.code.indexOf(this.activeData.str_id_list[i])] + '#'
          } else {
  ...
  methods: {
    ...
    subBackSelect (val) {
      if (val.radio == '1') {
        this.select_item.value = val.value
      } else {
        this.select_item.value = val
      }
    ...
    select_n_click (item) {
      ...
      this.select_num_flag += 1
      this.is_select = !this.is_select
      // this.other()
    },
    other () {
      this.select_item.value = Math.random()
    },
      ...
      photoCompress(this.newFile, {
        ...
        _this.$server.post(posturl, createvalue['fd']).then(data => {
          ...
          setTimeout(function () {
            ...
            _this.activeData.placeholder = _this.imgurl + data.urlpath
            _this.activeData.value = data.doccode
