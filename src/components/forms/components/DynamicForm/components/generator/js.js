...
function buildAttributes (el, dataList, ruleList, optionsList, methodList, propsList, uploadVarList, watchFuncList, tableRefs) {
  ...
  if (el.action && el.tag === 'el-upload') {
    uploadVarList.push(
      `field${el.idx}Action: '${el.action}',
      field${el.idx}fileList: [],`
    )
...
function buildOptions (conf, optionsList) {
  ...
  const str = `field${conf.idx}Options: ${JSON.stringify(conf.options)},`
...
function buildProps (conf, propsList) {
  ...
  const str = `field${conf.idx}Props: ${JSON.stringify(conf.props.props)},`
  propsList.push(str)
}
...
function buildBeforeUpload (conf) {
  ...
  const str = `field${conf.idx}BeforeUpload(file) {
    ${rightSizeCode}
    ${acceptCode}
