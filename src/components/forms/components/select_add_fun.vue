<template>
  <div class="formula-outer2">
    <el-dialog title="数值填写" :visible.sync="my_is_func" width="800px">
      <div>
        <div>
          <div>
            <div>
              <el-form class="radio_form" label-width="100px">
                <el-form-item label="选项数值类型">
                  <el-radio v-model="select_radio" label="1">数值</el-radio>
                  <el-radio v-model="select_radio" label="2">公式计算</el-radio>
                </el-form-item>
                <el-form-item v-if="select_radio == '1'" label="选项数值">
                  <el-input
                    placeholder="请填写选项数值"
                    class="width_240"
                    v-model="num_select"
          ...
          >
            ...
            <div>
              <div class="editor-outer">
                <div class="x-formula-editor_select">
                  <div class="formula-head">
                    <span class="formula-name">{{ func_name.label }}</span>
                ...
                <div class="flex">
                  <div class="fields-list">
                    ...
                    <ul class="field-list" style="display: block">
                      ...
                      >
                        <!-- <li>
                      {{item.Idx}}-{{funclist.idx}}{{item.C == 'select_number' && item.Is == true&& item.Idx!=funclist.idx}}
                    </li> -->
                        <!-- <li
                          v-if="
                            item.C == 'select_number' &&
                            item.Is == true &&
                            item.Idx != funclist.idx
                          "
                          :name="item.Code"
                          class="field"
          ...
          <div class="dialog-footer">
            <el-button @click="my_is_func = false" size="small"
              >取 消</el-button
            >
...
export default {
  ...
  watch: {
    code (val) {
      ...
      // fml1.FormulaEditor.insertField(a, true)
    },
    is_func (val) {
      this.my_is_func = val
      let that = this
      ...
      if (val) {
        ...
        console.log(this.select_num_flag)

        this.$nextTick(function () {
          if (this.select_num_flag == 1) {
            fml1.FormulaEditor._init();
          }
          console.log(this.func_name)
          console.log(typeof this.func_name.value)
          fml1.FormulaEditor.setValue()
          if (typeof this.func_name.value == 'object') {
            this.select_radio = '2'
            fml1.FormulaEditor.setValue()

            this.decimal_point = this.func_name.value.decimal_point
            this.outcome_state = this.func_name.value.outcome_state
            for (let i in that.func_name.value.strlist) {
              console.log('sdf')
              if (that.func_name.value.str_id_list[i] <= 'Z' && that.func_name.value.str_id_list[i] >= 'A') {

                let a = {
                  id: that.func_name.value.str_id_list[i],
                  name: that.func_name.value.str_id_list[i],
                  text: that.func_name.value.strlist[i]
                }
                // console.log(this.editor)
                fml1.FormulaEditor.insertField(a, true);//如果是当前表单的工作项 则不需要第二个参数
              } else {

                fml1.FormulaEditor.insertOtheritem(that.func_name.value.strlist[i]);
              }
            ...
            this.formulaId = ''
            this.formulaStr = ''
            this.num_select = this.func_name.value
  ...
  methods: {
    ...
    addOtherClick (val) {
      console.log(val)
      fml1.FormulaEditor.insertOtheritem(val.name);
    },
    addStrClick (val) {
    ...
    checkFunc () {
      let others = fml1.FormulaEditor.getValue()
    ...
    funcSubmit () {
      let $this = this
      if (this.select_radio == '1') {
        console.log(this.select_radio)
        console.log(this.num_select)
        if (this.num_select == '') {
          this.$alert('数值未填写！', '提示', {
            confirmButtonText: '确定',
          });
        ...
        if (!/^[+-]?(0|([1-9]\d*))(\.\d+)?$/g.test(this.num_select)) {
          this.$alert('数值请填写数字！', '提示', {
            confirmButtonText: '确定',
          });
          return false;
        }

        $this.$emit('subBack', { radio: '1', value: this.num_select })
        $this.my_is_func = false
        return false;
      }
      ...
      $this.$server.post(posturl, createvalue['fd']).then(() => {
        ...
        console.log(this.str_id_list)
        $this.$emit('subBack', { radio: '2', strlist: this.strlist, str_id_list: this.str_id_list, formulaStr: this.strlist, formulaId: this.str_id_list, decimal_point: this.decimal_point, outcome_state: this.outcome_state })
  ...
  mounted () {
    ...
    this.$nextTick(() => {
      ...
      fml1.Utils = {
        ...
        isEmpty: function (a) {
          return "" === a || fml1.Utils.isNull(a)
        },
        isBlank: function (a) {
          return fml1.Utils.isNull(a) || "" === a.trim()
        },
        isNull: function (a) {
          return null == a
        },
        ...
        num2Str: function (a, b) {
          if (fml1.Utils.isEmpty(a))
            return "";
          var c = a + "";
          if (fml1.Utils.isEmpty(b))
            return c;
          var d = /\[Num0\]/;
          if (d.test(b))
            return b.replace(d, c);
          if (d = /\[Num1\]/, d.test(b))
            return b.replace(d, fml1.Utils._num2CN(c, !1));
          if (d = /\[Num2\]/, d.test(b))
            return b.replace(d, fml1.Utils._num2CN(c, !0));
          d = /[#0]+,?[#0]*\.?[#0]*%?/;
          var e = b.match(d);
          if (e && e.length > 0) {
            var f = e[0];
            return c = fml1.Utils._numberFormat(a, f),
              b.replace(d, c)
          }
          return b
        },
        _numberFormat: function (a, b) {
          ...
          if (/,/.test(h))
            ...
            var q = h.match(/[0]+[0#]*$/);
            q && q.length > 0 && (o[0] = fml1.Utils.leftPad(p, q[0].length, "0"))
          }
          return o.join(".") + c
        },
        ...
        date2Str: function (a, b) {
          ...
          if (c > 0) {
            for (var e = b.charAt(0), f = 0, g = e, h = 1; h < c; h++) {
              var i = b.charAt(h);
              e !== i ? (d += fml1.Utils._compileDateFormat({
                char: e,
                str: g,
                len: h - f
              }, a), e = i, f = h, g = e) : g += i
            }
            d += fml1.Utils._compileDateFormat({
              char: e,
              str: g,
              len: c - f
            }, a)
        ...
        _compileDateFormat: function (a, b) {
          ...
          switch (e) {
            ...
            case "y":
              ...
              break;
            case "M":
              c = d > 2 ? Date._MN[b.getMonth()] : d < 2 ? b.getMonth() + 1 : fml1.Utils.leftPad(b.getMonth() + 1 + "", 2, "0");
              break;
            case "d":
              c = d > 1 ? fml1.Utils.leftPad(b.getDate() + "", 2, "0") : b.getDate();
              break;
            case "h":
              var f = b.getHours() % 12;
              0 === f && (f = 12),
                c = d > 1 ? fml1.Utils.leftPad(f + "", 2, "0") : f;
              break;
            case "H":
              c = d > 1 ? fml1.Utils.leftPad(b.getHours() + "", 2, "0") : b.getHours();
              break;
            case "m":
              c = d > 1 ? fml1.Utils.leftPad(b.getMinutes() + "", 2, "0") : b.getMinutes();
              break;
            case "s":
              c = d > 1 ? fml1.Utils.leftPad(b.getSeconds() + "", 2, "0") : b.getSeconds();
              break;
            case "a":
        ...
        pick: function (a, b) {
          var c = {};
          return fml1.Utils.forEach(b, function (b, d) {
            d in a && (c[d] = a[d])
          }),
            c
        },
        applyFunc: function (a, b, c, d) {
          return fml1.Utils.isFunction(b) ? b.apply(a, c ? c : []) : d
        },
        ...
        flatten: function (a, b, c) {
          if (c || (c = []), a)
            for (var d = 0, e = a.length; d < e; d++) {
              var f = a[d];
              Array.isArray(f) ? fml1.Utils.flatten(f, b, c) : b && !b(f) || c.push(f)
            }
          return c
        },
        applyCss: function (a, b) {
          fml1.Utils.isEmpty(b) || (fml1.Utils.isString(b) ? a.addClass(b) : a.css(b))
        },
        getServerDate: function (a) {
          if (a && a.getResponseHeader) {
            var b = a.getResponseHeader("date");
            b && (fml1.STATIC._st = new Date(b).getTime(), fml1.STATIC._ct = (new Date).getTime())
          }
        },
        ...
        evalFormula: function (a) {
          var b = [];
          fml1.Utils.forEach(Object.keys(fml1.Formula), function (a, c) {
            b.push("var " + c + "=fml1.Formula." + c)
          });
          var it = b.join(";") + ";";
        ...
        createWidgetName: function () {
          console.log('00999')
          return "_widget_" + fml1.STATIC.IDBase++
        },
        formatFileSize: function (a) {
          return fml1.Utils.isNumber(a) ? a >= 1e9 ? (a / 1e9).toFixed(2) + " GB" : a >= 1e6 ? (a / 1e6).toFixed(2) + " MB" : (a / 1e3).toFixed(2) + " KB" : "æœªçŸ¥"
        },
        ...
        setPageTitle: function (a) {
          fml1.Utils.isEmpty(a) || (document.title = a)
        },
        ...
        copyToClipboard: function (a, b) {
          if (a && a.length) {
            ...
            document.body.removeChild(c),
              d && fml1.Utils.applyFunc(this, b, [], !1)
          }
        },
        getColorNumber: function (a) {
          return fml1.Utils.isEmpty(a) ? 1 : parseInt(a.toString().substring(0, 8), 16) % 6 + 1
        },
        getFieldAttr: function (b, c) {
          if (!fml1.Utils.isValueWidget(b.widget.type))
            return null;
          ...
          switch (b.widget.type) {
            case "subform":
              var e = [];
              fml1.Utils.forEach(b.widget.items, function (d, f) {
                a.extend(f, {
                  formId: b.formId
                });
                var g = fml1.Utils.getFieldAttr(f, c);
                g && (a.extend(g, {
        ...
        getFieldInfoByFormula: function (a) {
          ...
          return fml1.Utils.forEach(d, function (a, c) {
        ...
        getFieldInfoById: function (a) {
          ...
          var c = a.match(/(#[0-9a-f]+)(@[0-9a-f]+)?/),
            d = ["", "entryId", "appId"];
          return fml1.Utils.forEach(c, function (a, c) {
        ...
        onWPSPageUnload: function (a, b) {
          a ? fml1.Msg.alert({
            ...
            text4Cancel: "å–æ¶ˆ",
            onOk: function () {
              fml1.Utils.applyFunc(this, b, [], !1)
            }
          }) : fml1.Utils.applyFunc(this, b, [], !1)
        },
        fileDownload: function (a, b) {
          if (fml1.Utils.isWpsWebView()) {
            var c = {
              method: "downloadUrl",
              url: a,
              filename: b
              ...
              d = b && b.split(".").pop();
            c.filter = "(*." + (d || "*") + ")",
              fml1.Utils.callWPSAPI(c)
          } else
            fml1.Utils.redirectTo(a)
        },
        ...
        dt: function (a, b) {
          if (a) {
            ...
            fml1.Utils.isEmpty(b) || c.push("ext=" + b),
              fml1.STATIC.user && fml1.STATIC.user.username && c.push("u=" + fml1.STATIC.user.username);
            var d = new Image;
            d.src = fml1.CONFIG.HOST.TRACK_HOST + "/log?" + c.join("&")
          }
        }
      }
      fml1.Formula = {
        AND: function () {
          for (var a = fml1.Utils.flatten(arguments), b = 0, c = a.length; b < c; b++)
            if (!a[b])
              return !1;
          return !0
        },
        ...
        XOR: function () {
          for (var a = 0, b = fml1.Utils.flatten(arguments), c = 0, d = b.length; c < d; c++)
            b[c] && a++;
          return !!(1 & Math.floor(Math.abs(a)))
        },
        CONCATENATE: function () {
          for (var a = fml1.Utils.flatten(arguments), b = 0; (b = a.indexOf(!0)) > -1;)
            a[b] = "TRUE";
        ...
        LEFT: function (a, b) {
          return b = fml1.Utils.isEmpty(b) ? 1 : b,
            a ? a.substring(0, b) : ""
        },
        LEN: function (a) {
          return fml1.Utils.isString(a) ? a ? a.length : 0 : a && a.length ? a.length : 0
        },
        LOWER: function (a) {
          return fml1.Utils.isString(a) ? a ? a.toLowerCase() : a : ""
        },
        REPLACE: function (a, b, c, d) {
          return fml1.Utils.isNumber(b) && fml1.Utils.isNumber(c) ? (a = a || "", d = d || "", a.substr(0, b - 1) + d + a.substr(b - 1 + c)) : a
        },
        REPT: function (a, b) {
          return b = b || 0,
        ...
        SEARCH: function (a, b, c) {
          var d;
          return fml1.Utils.isString(a) && fml1.Utils.isString(b) ? (c = fml1.Utils.isNull(c) ? 0 : c, d = b.toLowerCase().indexOf(a.toLowerCase(), c - 1) + 1) : 0
        },
        SPLIT: function (a, b) {
          return fml1.Utils.isString(a) ? a.split(b) : []
        },
        TRIM: function (a) {
          return fml1.Utils.isString(a) ? a.replace(/ +/g, " ").trim() : ""
        },
        UPPER: function (a) {
          return fml1.Utils.isString(a) ? a.toUpperCase() : ""
        },
        MID: function (a, b, c) {
          return a = a || "",
            fml1.Utils.isNumber(b) && fml1.Utils.isNumber(c) ? a.substr(b - 1, c) : a
        },
        AVERAGE: function () {
          for (var a = fml1.Utils.flatten(arguments, function (a) {
            return fml1.Utils.isNumber(a)
          }), b = a.length, c = 0, d = 0, e = 0; e < b; e++)
            c += a[e], d += 1;
          return c / d
        },
        COUNT: function () {
          return fml1.Utils.flatten(arguments).length
        },
        COUNTIF: function () {
        ...
        LARGE: function (a, b) {
          return a = fml1.Utils.flatten(a, function (a) {
            return fml1.Utils.isNumber(a)
          }),
            a.sort(function (a, b) {
              return b - a
        ...
        MAX: function () {
          var a = fml1.Utils.flatten(arguments, function (a) {
            return fml1.Utils.isNumber(a)
          });
        ...
        ABS: function (a) {
          return fml1.Utils.isNumber(a) ? Math.abs(a) : 0
        },
        ROUND: function (a, b) {
        ...
        CEILING: function (a, b) {
          ...
          return d > 0 && (e = -Math.floor(Math.log(d) / Math.log(10))),
            a >= 0 ? fml1.Formula.ROUND(Math.ceil(a / b) * b, e) : 0 === c ? -fml1.Formula.ROUND(Math.floor(Math.abs(a) / b) * b, e) : -fml1.Formula.ROUND(Math.ceil(Math.abs(a) / b) * b, e)
        },
        FLOOR: function (a, b) {
          if (0 === b)
            return 0;
          ...
          return c > 0 && (d = -Math.floor(Math.log(c) / Math.log(10))),
            a >= 0 ? fml1.Formula.ROUND(Math.floor(a / b) * b, d) : -fml1.Formula.ROUND(Math.floor(Math.abs(a) / b) * b, d)
        },
        INT: function (a) {
          return fml1.Utils.isNumber(a) ? Math.floor(a) : 0
        },
        LOG: function (a, b) {
          return b = void 0 === b ? 10 : b,
            fml1.Utils.isNumber(b) ? Math.log(a) / Math.log(b) : 0
        },
        MOD: function (a, b) {
          if (0 === b)
            return 0;
        ...
        SUM: function () {
          for (var a = 0, b = fml1.Utils.flatten(arguments, function (a) {
            return fml1.Utils.isNumber(a)
        ...
        FIXED: function (a, b) {
          return b = void 0 === b ? 0 : b,
            fml1.Utils.isNumber(b) && b >= 0 ? Number(a).toFixed(b) : ""
        },
        DATE: function () {
        ...
        TIMESTAMP: function (a) {
          return fml1.Utils.isDate(a) ? a.getTime() : 0
        },
        TODAY: function () {
          return new Date
        },
        ...
        SYSTIME: function () {
          var a = fml1.STATIC._st,
            b = (new Date).getTime() - fml1.STATIC._ct;
        ...
        DATEDELTA: function (a, b) {
          return fml1.Utils.isNumber(b) || (b = 0),
        ...
        TEXT: function (a, b) {
          return fml1.Utils.isNull(a) ? "" : fml1.Utils.isDate(a) && !fml1.Utils.isEmpty(b) ? fml1.Utils.date2Str(a, b) : fml1.Utils.num2Str(a, b)
        },
        VALUE: function (a) {
          return fml1.Utils.isEmpty(a) ? 0 : isNaN(a) ? 0 : parseFloat(a)
        },
        UUID: function () {
          return fml1.Utils.UUID()
        },

        ISEMPTY: function (a) {
          return fml1.Utils.isObjectEmpty(a)
        },
        GETUSERNAME: function () {//需修改
          return fml1.STATIC.user ? fml1.STATIC.user.nickname : ""
        }
      }
      fml1.FormulaUsage = [{
        category: "数学函数",
      ...
      fml1.OB = function (a) {
        ...
        _beforeInit: function () {
          fml1.Utils.applyFunc(this, this.options.onBeforeInit, [], !1)
        },
        _afterInit: function () {
          fml1.Utils.applyFunc(this, this.options.onAfterInit, [], !1)
        },
      });
      fml1.Widget = fml1.extend(fml1.OB, {
        _defaultConfig: function () {
          return $.extend(fml1.Widget.superclass._defaultConfig.apply(this, arguments), {
            widgetName: "",
            baseCls: "",
            customCls: null,
            enable: !0,
        ...
        _init: function () {
          fml1.Widget.superclass._init.apply(this, arguments),
            this._initRoot(),
        ...
        _afterInit: function () {
          this._initElementSize(),
            ...
            fml1.Widget.superclass._afterInit.apply(this, arguments)
        },
        _initRoot: function () {
          var a = this.options;
        ...
        _initNameEffects: function () {
          var a = this.options;
          a.widgetName || (a.widgetName = "_widget_" + fml1.STATIC.IDBase++),
            this.element.attr({
        ...
        doResize: function (a) {
          ...
          a && (b.width = a.width, b.height = a.height),
            fml1.Utils.isEmpty(b.width) || this.element.css({
              width: b.width
            }),
            fml1.Utils.isEmpty(b.height) || this.element.css({
              height: b.height
            })
        },
        destroy: function () {
      ...
      fml1.FormulaEditor = {
        ...
        _defaultConfig: function () {
          return a.extend(fml1.FormulaEditor.superclass._defaultConfig.apply(), {
            baseCls: "x-formula-editor_select",
            keywords: [],
            text: "学号",
            hasFunction: !0,
            labelMap: null
        ...
        _init: function () {
          // fml1.FormulaEditor.superclass._init.apply(this, arguments);
          var b = {
            baseCls: "x-formula-editor_select",
            keywords: [],
            text: $this.code,
        ...
        getValue: function () {
          ...
          console.log(e)
          return fml1.Utils.forEach(e, function (e, f) {
            var g = [];
            var o = []
            console.log(f)
            console.log()
            fml1.Utils.forEach(a(f).children("span"), function (b, e) {
        ...
        setValue: function (a) {
          ...
          if (a) {
            var f = a.split("\n");
            fml1.Utils.forEach(f, function (a, b) {
              var f = "",
                g = b.split(/(\$[0-9a-zA-Z\._#@]+)/g);
              fml1.Utils.forEach(g, function (b, d) {
                if (/^\$(_widget_|_formula_|ext)/.test(d)) {
                  var g;
                  fml1.Utils.startWith(d, "$ext") ? g = "扩展字段" : c.labelMap && (g = c.labelMap[d]);
                  var h = !1;
                  fml1.Utils.isNull(g) && (g = "无效字段", h = !0);
          ...
          this.editor.setValue(d.join("\n")),
            fml1.Utils.forEach(e, function (a, c) {
              b._markField(c)
            })
        }
      };
      ...
      CodeMirror.fomulaContext = Object.keys(fml1.Formula);
      ...
      $(".subbtn").click(function () {
        let val = $('.x-formula-editor_select').childNodes
        console.log(fml1)
        console.log(this.editor)
        let others = fml1.FormulaEditor.editor.getValue()
        console.log(others)
        console.log(fml1.FormulaEditor.getValue())
        // console.log(fml1.FormulaEditor.editor.options())
        // fml1.FormulaEditor.getValue('$_widget_1493093397868#')
        var a = {
          id: '123',
          name: 'ddd',
          text: '数字'
      ...
      $(".formula-category .formula-item").mouseover(function () {
        var name = $(this).text(), list = fml1.FormulaUsage, intro = $(".formula-intro .intro-wrapper");
        var currentfml1;
        ...
        if (currentfml1) {
          intro.html('<li class="intro"><span class="formula-name">' + currentfml1.name + '</span>' + (currentfml1.intro.indexOf(currentfml1.name) > -1 ? currentfml1.intro.split(currentfml1.name)[1] : currentfml1.intro) + '</li><li class="usage">用法：<span class="formula-name">' + currentfml1.name + '</span>(' + currentfml1.usage.split("(")[1] + '</li><li class="example">示例：' + currentfml1.example + '</li>')
        }
      }).click(function () {
        fml1.FormulaEditor.insertFormulaitem($(this).text());
      })
