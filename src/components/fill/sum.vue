    ...
    <div class="top_form">
      ...
      <span class="right">
        <router-link
          :to="{
            name: 'formlistFill',
            query: { id: this.$route.query.id, k: Date.now() },
          }"
          class="margin_l_15"
        >
          <el-button
...
export default {
  ...
  methods: {
    ...
    getgather () {
      ...
      let posturl = $this.$apilist['personalgather'] + '?signature=' + keyvalue;
      // return false;
      console.log(payload)
      this.gather_list = []
      this.rule_list = []
      ...
      $this.$server.post(posturl, createvalue['fd']).then((data) => {
        ...
        this.gather_list = data.data.form
    ...
    exportExcel () {
      ...
      let payload = {
        corpid: this.$store.state.corpid,
        userid: this.$store.state.userid,
        task_code: afterEncrypt(this.code)
      }
      let createvalue = $this.$createObj(payload);
      var keyvalue = createvalue['keyvalue'];
