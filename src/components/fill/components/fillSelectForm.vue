      ...
      <div class="">
        <el-form label-position="top">
          <el-form-item label="表单">
            <el-select v-model="formcode" @change="descSelectChange()">
              <el-option-group
                v-for="group in Object.keys(formlist)"
                :key="group"
                :label="group"
              >
                <el-option
                  v-for="item in formlist[group]"
                  :key="item.Code"
                  :label="item.Title"
                  :value="item.Code"
                >
...
export default {
  ...
  methods: {
    ...
    descSelectChange () {
      ...
      // this.fillcode = this.fillcodetitle.split('===')[0]
      // this.filltitle = this.fillcodetitle.split('===')[1]
      // this.gather(this.fillcode)
      // this.gatherlist(this.fillcode)
      // this.allmaindatasbymissioncode(this.fillcode)
      this.getformdetail(this.formcode)
    },
    getformdetail (code) {
      let $this = this
      ...
      $this.$server.post(posturl, createvalue['fd']).then((data) => {
        ...
        this.form_data1 = JSON.parse(JSON.stringify(data.data))
        this.form_fields = this.form_data1.fields
        this.form_fields_select = []
        for (let i in this.form_fields) {
          if (this.form_fields[i].tagIcon == 'select_number') {
            this.form_fields_select.push(this.form_fields[i])
          }
        }
        this.is_form = true
    ...
    countselected (item) {
      ...
      for (let i in this.form_fields) {
        if (this.form_fields[i].required) {
          ...
          if (this.form_fields[i].tagIcon == 'number') {
            ...
            if (this.form_fields[i].is_counted.split(']}__').length < 2) {
              formlist.push({ "field_code": this.form_fields[i].filed_code, opt_label: this.form_fields[i].is_counted, "field_label": this.form_fields[i].label, "join_gather": false, "is_counted": false, "enable_greater": this.form_fields[i].enable_greater, "enable_assign": this.form_fields[i].enable_assign, "value": parseFloat(this.form_fields[i].is_counted.split('__')[0]) })

            } else {
              // formlist.push({ "field_code": this.form_fields[i].filed_code, opt_label: this.form_fields[i].is_counted, "field_label": this.form_fields[i].label, "join_gather": false, "is_counted": false, "enable_greater": this.form_fields[i].enable_greater, "enable_assign": this.form_fields[i].enable_assign, "value": parseFloat(this.form_fields[i].join_gather) })
              if (typeof this.form_fields[i].join_gather == 'number') {
                formlist.push({ "field_code": this.form_fields[i].filed_code, opt_label: this.form_fields[i].is_counted, "field_label": this.form_fields[i].label, "join_gather": false, "is_counted": false, "enable_greater": this.form_fields[i].enable_greater, "enable_assign": this.form_fields[i].enable_assign, "value": parseFloat(this.form_fields[i].join_gather) })
      ...
      let other_fun = JSON.parse(item.is_counted.split('__')[0]).str_id_list
      console.log(other_fun)
      // return false
      let $this = this
      let payload = {
        corpid: this.$store.state.corpid,
        ...
        file_info: JSON.stringify(formlist),
        formula_method: JSON.stringify(other_fun)
      }

      let createvalue = $this.$createObj(payload);
      var keyvalue = createvalue['keyvalue'];
      let posturl = $this.$apilist['countselected'] + '?signature=' + keyvalue;
      // return false;
      $this.$server.post(posturl, createvalue['fd']).then((data) => {
        console.log(data)
        item.join_gather = data.data
        this.subClickTwo()
        // this.subClick()
      }).catch(err => {
        console.log(err)
    ...
    subClick () {
      for (let j in this.form_fields_select) {
        if (this.form_fields_select[j].is_counted.split(']}__').length > 1) {
          console.log('------------------------------------------')
          this.countselected(this.form_fields_select[j])
        }
      }
