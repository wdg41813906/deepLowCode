...
export default {
  ...
  methods: {
    gourl () {
      this.$router.push({ name: 'sumendFill', query: { id: this.$route.query.id, name: this.$route.query.name } })
    },
    otherDetailBack () {
    ...
    goClick (item) {
      console.log(item)
      this.$router.push({ name: 'fillendFill', query: { name: item.Title, taskname: this.$route.query.name, id: beforeEncrypt(item.Code), taskcode: this.$route.query.id } })
    },
    tabClick (num) {
      if (num == 1) {
        this.is_tab = true
    ...
    allmaindatabyformcodeanduserid () {
      let $this = this
      this.task_code = afterEncrypt(this.$route.query.id)
      this.task_name = afterEncrypt(this.$route.query.name)
      let payload = {
        corpid: this.$store.state.corpid,
        userid: this.$store.state.userid,
        task_code: this.task_code
