    ...
    excelupload () {
      ...
      _this.$server.post(posturl, createvalue['fd']).then((data) => {
        ...
        if (data.data.f_data == null) {
          ...
          _this.$router.push({ name: 'formdataATask', query: { id: this.$route.query.id, taskcode: this.$route.query.taskcode, taskname: this.$route.query.taskname, name: this.$route.query.name } })
        } else {
          console.log(data)
