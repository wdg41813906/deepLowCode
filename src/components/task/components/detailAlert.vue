...
export default {
  ...
  methods: {
    ...
    getimgurl (key, item) {
      ...
      // console.log(item)
      // item.enable_greater.push(this.imgurl + item.url)
      // return item
      // let createvalue = $this.$createObj(payload);
      // var keyvalue = createvalue['keyvalue'];
      // let posturl = $this.$apilist['qiniudown'] + '?signature=' + keyvalue;
    ...
    is_add (val) {
      ...
      if (val) {
        ...
        for (let i in this.detail.FormFieldInfos) {
          if (this.detail.FormFieldInfos[i].opt_label == 'image') {
            this.enable_assign = []
            ...
            for (let j in this.detail.FormFieldInfos[i].value) {
              ...
              this.getimgurl(this.detail.FormFieldInfos[i].value[j], this.detail.FormFieldInfos[i])
