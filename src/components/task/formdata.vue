...
export default {
  ...
  methods: {
    ...
    gourl () {
      ...
      this.$router.push({ name: 'importTask', params: { other: 1 }, query: { id: this.$route.query.id, taskcode: this.$route.query.taskcode, taskname: this.$route.query.taskname, name: this.$route.query.name } })
    },
    goimg (key) {
      window.open(key);
      return false;
